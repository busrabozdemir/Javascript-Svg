...
function Process(_middle_x, _middle_y, _size, _text, _text_size, _description, _fill_color, _stroke_color, _text_color) {
    ...
    this.points = [false, false, false, false]; //bağlantı noktalarının durumu
    ...
    //Dizi ilk parametresi: 0_top, 1_bottom, 2_left, 3_right
    //Array ikinci parameter: 0_x, 1_y
...
//Bakış Sembollerinin Başlatılması ve Kontrolü
...
function Decision(_middle_x, _middle_y, _size, _text, _text_size, _description, _fill_color, _stroke_color, _text_color) {
    ...
    //Kararda genişlik ve yükseklik eşittir
    this.probable_x_y[0][0] = this.middle_x;
...
//Akış Sembollerinin Dallanması ve Kontrolü son

//Giriş ve Çıkış Sembolleri BAŞLAT
...
//Sembol giriş çıkış sonu


function doClick_Object(evt) {
    //Tüm svg rect, çokgenler, yollar, daireler, çizgiler ...
...
function Line(object1, object2, position1, position2, _text, _text_size, _description, _stroke_color, _text_color, _arrow) {
    ...
    if (!(typeof object1 === 'undefined' || object1 == null) && !(typeof object2 === 'undefined' || object2 == null)) {
        ...
        //this.line_start_end_x_y = [[0, 0], [0, 0]];//line - bu hattı bağlamak için yeni bir çizgi çizmek için koordinatları başlatın ve bitirin
        var selection1 = [0, 0];
        var selection2 = [0, 0];
    ...
    // Array ilk parametresi: 0_top, 1_bottom, 2_left, 3_right
    this.LinesData = "";

    this.Draw = function () {
        if (object1 == null || object2 == null)
            return ""; //Nesne hatası böylece hiçbir çizgi çizme 
        ...
        temp = findObjectArrayLinecollision(temp); //tüm nesneler       
        var temp2 = findLineArraycollision(temp); //tüm hatlar
        //document.getElementById("desc").innerHTML += "Burdayım <br />" + temp;
...
function doClick_Line(evt) {
    //Tüm svg rect ve polylines seçtikleri renkler ile boya ...
...
writeConsoleText("OKKK", "information");
    ...
    if (theLine[0].nodeName == "tspan")
        theLine = evt.target.parentElement.parentElement.children; //tspan etiketine tıklarsa, parent iki üstte
    if (theLine[0].nodeName == "polyline") {
        theLine[0].setAttribute("style", "fill:none;stroke:" + highlight_color + ";stroke-width:3");
        if (theLine[0].hasAttribute("marker-end"))//ok ya da değil
    ...
    for (var i = 0; i < this.o1_probable_x_y.length; i++) {
        for (var j = 0; j < this.o2_probable_x_y.length; j++) {
            if (this.o1_points[i] == false && this.o2_points[j] == false) {
                ...
                //Nesnelerin seçimini değiştir
                if (yeni_dif < dif) {
                    dif = yeni_dif;
                    position1 = i;
                    position2 = j;
                }//noktalar kullanılmazsa
            }//if
        }// object2 için
    }// object1 için
            ...
            if (col_start != -1 && col_end != -1) {
                ...
                for (var j = 0; j < line_arrayx.length; j++) {
                    ...
                    if (j < col_start || j > col_end) {
                        ...
                        if (j == col_end) { //sadece bir tekrar için   
